        ...
        for line in lines:
            ...
            if isinstance(entry['caption'], list):
                ...
                for i, s in enumerate(caption_list):
                    old_caption = s.strip()
                    new_caption = convert_caption_with_retry(old_caption)
                    caption_list[i] = new_caption
                    print("[" + str(r) + " / " + str(len(lines)) + "] " + old_caption + " ===> " + new_caption)
            else:
                old_caption = entry['caption']
                new_caption = convert_caption(old_caption)
...
def testCommand(caption, gpu):
    ...
    for nTry in range(20):
        ...
        print("N(" + str(nTry) + "): " + new_caption)
        if not needs_refinining(new_caption, caption):
            print("Caption passed checks")
            break
...
def main():
    ...
    parser.add_argument("command", help='command to execute [caption, refine]')
    ...
    match args.command:
        ...
        case "test":
            testCommand(args.caption, args.gpu)
        case _:
